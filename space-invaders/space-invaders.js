(function () {
  ...
  Class.extend = function (prop) {
    ...
    Class.prototype.varructor = Class;
...
var Player = SheetSprite.extend({
  ...
  shoot: function () {
    var bulvar = new Bulvar(
      this.position.x,
      this.position.y - this.bounds.h / 2,
      1,
      1000,
    );
    this.bulvars.push(bulvar);
  },
  ...
  handleInput: function () {
    ...
    if (wasKeyPressed(SHOOT_KEY)) {
      if (this.bulvarDelayAccumulator > 0.5) {
        this.shoot();
        this.bulvarDelayAccumulator = 0;
      }
    }
  ...
  updateBulvars: function (dt) {
    for (var i = this.bulvars.length - 1; i >= 0; i--) {
      var bulvar = this.bulvars[i];
      if (bulvar.alive) {
        bulvar.update(dt);
      } else {
        this.bulvars.splice(i, 1);
        bulvar = null;
      }
    }
  },
  ...
  update: function (dt) {
    this.bulvarDelayAccumulator += dt;
    ...
    this.updateBulvars(dt);
  },

  draw: function (resized) {
    this._super(resized);

    for (var i = 0, len = this.bulvars.length; i < len; i++) {
      var bulvar = this.bulvars[i];
      if (bulvar.alive) {
        bulvar.draw(resized);
      }
...
var Bulvar = BaseSprite.extend({
  init: function (x, y, direction, speed) {
    this._super(bulvarImg, x, y);
...
var Enemy = SheetSprite.extend({
  ...
  update: function (dt) {
    ...
    if (this.bulvar !== null && this.bulvar.alive) {
      this.bulvar.update(dt);
    } else {
      this.bulvar = null;
    }
  },
...
function preDrawImages() {
  ...
  bulvarImg = new Image();
  bulvarImg.src = canvas.toDataURL();
}
...
function resolveBulvarEnemyCollisions() {
  var bulvars = player.bulvars;

  for (var i = 0, len = bulvars.length; i < len; i++) {
    var bulvar = bulvars[i];
    for (var j = 0, alen = aliens.length; j < alen; j++) {
      var alien = aliens[j];
      if (checkRectCollision(bulvar.bounds, alien.bounds)) {
        alien.alive = bulvar.alive = false;
...
function resolveBulvarPlayerCollisions() {
  for (var i = 0, len = aliens.length; i < len; i++) {
    var alien = aliens[i];
    if (
      alien.bulvar !== null &&
      checkRectCollision(alien.bulvar.bounds, player.bounds)
    ) {
      if (player.lives === 0) {
        hasGameStarted = false;
      } else {
        alien.bulvar.alive = false;
        particleManager.createExplosion(
          player.position.x,
          player.position.y,
          "green",
...
function resolveCollisions() {
  resolveBulvarEnemyCollisions();
  resolveBulvarPlayerCollisions();
}
